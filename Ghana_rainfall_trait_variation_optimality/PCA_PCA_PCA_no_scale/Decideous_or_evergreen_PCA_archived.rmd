...
### Stat looks good
iris.pca <- PCA(PCA_table_complete_hydro[,c('Huber_value','Chi_value','Ks_value','Vcmax_ACi_25C')], graph = FALSE,scale.unit = F)
...
# Part two, without turgor loss point (This is disgusting chaos, we should give up this part)
...
#I am doing this because Turgor and Vcmax were done on largely different species



PCA_table_no_TLP <-PCA_table %>% 
  mutate(Phenology=stringi::stri_replace_all_charclass(Phenology, "\\p{WHITE_SPACE}", ""))%>%
  mutate(Phenology=recode(Phenology,"?ed"='evergreen',"ed"='evergreen',"de"='deciduous'))%>%
  select(Species_site,Huber_value, Ks_value, TwigDensity,site,Phenology)%>% 
  column_to_rownames("Species_site")%>%
  missRanger(num.trees = 500)



### Stat looks good
iris.pca <- PCA(PCA_table_no_TLP[,1:3], graph = FALSE)
var <- get_pca_var(iris.pca)
summary(iris.pca)
...
fviz_pca_biplot(iris.pca, 
                col.ind = PCA_table_no_TLP$site, palette = "jco", 
                addEllipses = TRUE, label = "var",
                col.var = "black", repel = TRUE,
                legend.title = "Species")
...
fviz_pca_ind(iris.pca,
             col.ind = PCA_table_no_TLP$site, # color by groups
             palette = "jco",
             addEllipses = TRUE, # Concentration ellipses
             ellipse.type = "confidence",
             legend.title = "site",
...
fviz_pca_biplot(iris.pca, 
                col.ind = PCA_table_no_TLP$Phenology, palette = "jco", 
                addEllipses = TRUE, label = "var",
                col.var = "black", repel = TRUE,
                legend.title = "Species")
...
fviz_pca_ind(iris.pca,
             col.ind = PCA_table_no_TLP$Phenology, # color by groups
             palette = "jco",
             addEllipses = TRUE, # Concentration ellipses
             ellipse.type = "confidence",
             legend.title = "Groups",
