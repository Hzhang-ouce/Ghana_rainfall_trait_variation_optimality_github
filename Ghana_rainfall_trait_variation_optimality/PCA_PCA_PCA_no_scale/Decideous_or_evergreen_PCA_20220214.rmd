---
title: "chapter one PCA on hydraulic trait"
author: "Huanyuan and many others"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: yes
...
#setwd("C:/Users/Shirley/Documents/GitHub/Ghana_rainfall_trait_variation_optimality_github/input_data")
...
Species_table<-read.csv("Census_data/Census_table_all_plots_Species_list.csv")
...
Trait_big_table<-drop_na(Trait_big_table,any_of('Species'))%>%  mutate(Turgor=log10(-Turgor))%>%
...
Trait_big_table<-drop_na(Trait_big_table,any_of('Ks')) # I added this line because varpart function won't work with NAs
...
**turgor loss point was not done on the same species as most of the other traits!**

**I tried to incorporate Turgor loss point, or Tree height but the pattern is getting messy.**

**Having Ci ca ratio seems to improve the pattern**

**The pattern is consistent for Asat and Vcmax (well, they are highly correlated)**

**it is also a poor idea to put Asat and Vcmax in one pca, it just skew the pattern**

The id is for example ANK_Anthocleista nobilis, so same species from different sites got different ID


# Part one, Vcmax,

**Lets go with Vcmax which seems to be the best pattern, and then say Asat and Amax has more value, we can reproduce same pattern with Asat and Amax**
...
PCA_table<-full_join(Huber1,VCmax1,by="Species_site")%>%
#  full_join(Tree_height1,by="Species_site")%>%
...
PCA_table_complete_hydro<-PCA_table_complete%>%
  dplyr::select(Huber_value,Vcmax_ACi_25C,Ks_value,Species_site,Chi_value)%>%
...
### Stat looks good
iris.pca <- PCA(PCA_table_complete_hydro[,c('Huber_value','Chi_value','Vcmax_ACi_25C','Ks_value')], graph = FALSE,scale.unit = F)
...
ggsave(filename = 'PCA_variable.png', device = 'png',width = 4, height = 4)
### See it by site

fviz_pca_biplot(iris.pca, 
                col.ind = PCA_table_complete$site, palette = "jco", 
                addEllipses = TRUE, label = "var",
                col.var = "black", repel = TRUE,ellipse.type = "confidence",
                legend.title = "Species")
# Save the plot
png("P1_site_ori.png", width = 488, height = 301)
fviz_pca_biplot(iris.pca, 
                col.ind = PCA_table_complete$site, palette = "jco", 
                addEllipses = TRUE, label = "var",
                col.var = "black", repel = TRUE,ellipse.type = "confidence",
...
fviz_pca_biplot(iris.pca, 
                ...
                ggtheme = theme_bw())
# Save the plot
png("P1_pheno_ori.png", width = 488, height = 301)
fviz_pca_biplot(iris.pca, 
                col.ind = PCA_table_complete$Phenology, palette = "jco", 
                pointshape = 16,
                addEllipses = TRUE, label = "var",
...
fviz_pca_ind(iris.pca,
             col.ind = PCA_table_complete$Phenology, # color by groups
             palette = "jco",
             addEllipses = TRUE, # Concentration ellipses
             ellipse.type = "confidence",
             legend.title = "Groups",
...
a <- as.data.frame(iris.pca$ind$coord)%>%
  mutate(Species_site=rownames(iris.pca$ind$coord))%>%
  left_join(PCA_table_complete, by="Species_site")

# Plot
Pheno_PCA <- ggplot(a, aes(Dim.1, Dim.2)) +
  geom_point(aes(color = Phenology)) +
  theme_bw() +
  scale_color_manual(values = c("#fc8d62","#66c2a5","#8da0cb"), labels = c("Deciduous", "Evergreen", "Other"))
Pheno_PCA
# Save the plot
png("P1_pheno.png", width = 488, height = 301)
Pheno_PCA
dev.off()

site_PCA <- ggplot(a, aes(Dim.1, Dim.2)) +
  geom_point(aes(color = site)) +
  theme_bw() +
  scale_color_manual(values = c("#e41a1c","#377eb8","#4daf4a"), name = "Site")
site_PCA
# Save the plot
png("P1_site.png", width = 488, height = 301)
site_PCA
dev.off()
...
fviz_pca_biplot(iris.pca, 
                ...
                legend.title = "Species")
# Save the plot
png("P2_site_ori.png", width = 488, height = 301)
fviz_pca_biplot(iris.pca, 
                col.ind = PCA_table_complete$site, palette = "jco", 
                ellipse.type = "confidence",
                addEllipses = TRUE, label = "var",
...
fviz_pca_biplot(iris.pca, 
                ...
                legend.title = "Species")
# Save the plot
png("P2_pheno_ori.png", width = 488, height = 301)
fviz_pca_biplot(iris.pca, 
                col.ind = PCA_table_complete$Phenology, palette = "jco", 
                ellipse.type = "confidence",
                addEllipses = TRUE, label = "var",
...
Pheno_PCA <- ggplot(a, aes(Dim.1, Dim.2)) +
  geom_point(aes(color = Phenology)) +
  theme_bw() +
  coord_cartesian(ylim = c(-0.6,0.5), xlim = c(-1,0.8)) +
  scale_color_manual(values = c("#fc8d62","#66c2a5","#8da0cb"),     labels = c("Deciduous", "Evergreen", "Other"))
Pheno_PCA
# Save the plot
png("P2_pheno.png", width = 488, height = 301)
Pheno_PCA
dev.off()
...
site_PCA <- ggplot(a, aes(Dim.1, Dim.2)) +
  geom_point(aes(color = site)) +
  theme_bw() +
  coord_cartesian(ylim = c(-0.6,0.6), xlim = c(-1.1,0.8)) +
  scale_color_manual(values = c("#e41a1c","#377eb8","#4daf4a"), name = "Site")
site_PCA
# Save the plot
png("P2_site.png", width = 488, height = 301)
site_PCA
dev.off()
...
# Part two, What if we combing all hydraulic shit into one arrow

```{r}
##------------------
##  conducting PCA with NA filling - try to bring in more species... 
##------------------
...
PCA_table_complete$Phenology<-as.factor(PCA_table_complete$Phenology)
PCA_table_complete_hydro<-PCA_table_complete%>%
  dplyr::select(Huber_value,Vcmax_ACi_25C,Ks_value,Species_site,Chi_value,Height_value)%>%
  mutate(Ks_star_Huver= Huber_value * Ks_value / Height_value) %>%
  left_join(species_basal)
...
### Stat looks good
iris.pca <- PCA(PCA_table_complete_hydro[,c('Ks_star_Huver','Chi_value','Vcmax_ACi_25C')], graph = FALSE,scale.unit = T)
var <- get_pca_var(iris.pca)
summary(iris.pca)
